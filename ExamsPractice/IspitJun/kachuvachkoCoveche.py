...
"""
Дадена е табла 5х9, каде што е поставено човече. Потребно е чоевчето со качуввање по ѕидот да стигне до врвот кој е означен со куќа.
Човечето може да се движи во три насоки горе, горе-десно, горе-лево за една или две позиции. Човечето може и да остане на моменталното поле.
Притоа човечето може да се наоѓа само на полиња кои поставен зелен осмоаголник. Полиња кои не се означени со осумаголник може да се прескокнат.
Човечето исто така не смее да излезе од таблата. Куќичката е подвижна и се движи лево десно. Таа може да застане на било кое поле
во редот кој се наоѓа. Куќичката на почетокот се движи во една насока. а кога ќе стигне до крајот на таблата ја менува насоката.
Единственото поло во најгорниот ред на кое може да застане човечето е она кое се наоѓа куќата.
...
class Rock(Problem):
    def __init__(self, allowed_pos, initial, goal=None):
    ...
    def house_mover(self, house_state):
        x, y, pos = house_state
        if pos == "levo":
            x -= 1
            if x == 0:
    ...
    def successor(self, state):
        # ((1,0), (1,8,desno))
        # problem = Rock(allowed, (man_pos, (house_pos[0], house_pos[1], direction)))
        successors = dict()
        man_x, man_y = state[0]
        house_x, house_y, house_pos = state[1]
        house_state = state[1]

        # Man stand still
        new_state = (state[0], state[1])
        if self.check_valid(new_state):
            new_house_pos = self.house_mover(house_state)
            successors["stoi vo mesto"] = (new_state, new_house_pos)

        # Move Up 1 and 2 steps
        for i in range(0, 2):
            new_state = ((man_x, man_y + i), self.house_mover(house_state))
            if self.check_valid(new_state):
                successors[f'pravo za {i}'] = ((man_x, man_y + i), self.house_mover(house_state))

            # Move Up  Right 1 and 2 steps
        for i in range(0, 2):
            new_state = ((man_x + i, man_y + i), self.house_mover(house_state))
            if self.check_valid(new_state):
                successors[f'gore-desno za {i}'] = ((man_x + i, man_y + i), self.house_mover(house_state))

            # Move Up  Levo 1 and 2 steps
        for i in range(0, 2):
            new_state = ((man_x - i, man_y + i), self.house_mover(house_state))
            if self.check_valid(new_state):
                successors[f'gore-levo za {i}'] = ((man_x - i, man_y + i), self.house_mover(house_state))
    ...
    def goal_test(self, state):
        man_x, man_y = state[0]
        house_x, house_y, dir = state[1]
        return man_x == house_x and man_y == house_y

    def h(self, node):
        man_x, man_y = node.state[0]
        house_x, house_y, dir = node.state[1]
        return (abs(house_x - man_x) + abs(house_y - man_y))/2
