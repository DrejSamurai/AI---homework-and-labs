...
if __name__ == '__main__':
    ...
    training_set = dataset[:int(0.85 * len(dataset))]
    training_x = [row[:-1] for row in training_set]
    training_y = [row[-1] for row in training_set]
    training_x = encoder.transform(training_x)

    testing_set = dataset[int(0.85 * len(dataset)):]
    test_x = [row[:-1] for row in training_set]
    test_y = [row[-1] for row in training_set]
    ...
    print(acc/len(testing_set))

    element = [el for el in input().split(' ')]

    encoded_entry = encoder.transform([element])
    predicted = classifier.predict(encoded_entry)[0]
    print(predicted)
