...
if __name__ == '__main__':
    train_set_1 = dataset[: int(len(dataset) * 0.3)]
    train_set_2 = dataset[int(0.3 * len(dataset)):int(0.6 * len(dataset))]
    train_set_3 = dataset[int(len(dataset) * 0.6):]
    ...
    the_list = [classifier1.predict(test_x)[0], classifier2.predict(test_x)[0], classifier3.predict(test_x)[0]]
    classes_list = [0, 1, 2]
    votes_dict = dict()

    for i in classes_list:
        votes_dict[i] = the_list.count(i)

    max_class = max(votes_dict, key=votes_dict.get)
    ...
    if ctr > 1:
        ...
        print(f'Glasovi: {votes_dict}')
        print(f'Predvidena klasa: {max_class}')
        #print(f'Ctr is: {ctr}')
        #print()